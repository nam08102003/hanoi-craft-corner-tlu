...
public class ProfileUserActivity extends AppCompatActivity {
    ...
    private void fetchUserData() {
        if (USER_ID == null || USER_ID.isEmpty() || USER_ID.equals("user_001")) { // Kiểm tra ID mẫu
            // Bạn có thể thêm điều kiện kiểm tra phức tạp hơn nếu USER_ID là một giá trị mặc định cần thay thế
            Log.w(TAG, "USER_ID có thể đang sử dụng giá trị mẫu. Hãy đảm bảo bạn đã thay đổi nó thành ID hợp lệ.");
            // Toast.makeText(this, "USER_ID mẫu, vui lòng kiểm tra code.", Toast.LENGTH_LONG).show();
